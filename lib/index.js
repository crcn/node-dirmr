...
module.exports = function(_dirs) {
	...
	_self.reset = function(dirs) {
		

		_seq.seq(function() {
			_targetDirs = toarray(dirs);
	...
	_self.join = function(destination, setAsDir) {
		...
		copyDirs(destination, function(dir) {
			...
			var next = this;
			_walkr.start(dir, destination, function(err) {
				if(err) return next(err);
				if(setAsDir) _targetDirs = toarray(destination);
				next();
			});
